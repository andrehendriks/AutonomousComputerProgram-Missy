...
(eval-when (:execute :compile-toplevel :load-toplevel)
  (import '(rs::class-precedence-list rs::class-direct-subclasses
            ...
            atms:assumptions atms:singletonp
            rs::revdolist rs::rules-affecting-locally
...
(defun map-core-slots (class fn &key (direct nil))
  ...
  (map nil
       #'(lambda (slot-definition &aux counted-slot-definition)
           (unless (excluded-slot-p slot-definition)
             (setq counted-slot-definition
                   (find-slot-definition class (slot-definition-name slot-definition)
  ...
  (with-accessors ((slot-name slot-definition-name)
    ...
    (setq complex (subtypep slot-type 'extended-object)
        name (if (and complex (eq slot-name slot-type))
                 ...
                 slot-name)
        type-name (cond (complex
                         (ensure-xml-type-name slot-type))
                        ((eq slot-type 'true-or-false)
                         'boolean)
                        (t slot-type)))
    (if count-slot
