...
(eval-when (:execute :compile-toplevel :load-toplevel)
  ...
  (import '(rs::class-direct-superclasses
            ...
            rs::ensure-range-class rs::make-numeric-range
  ...
  (export '(defnamespace make-namespace ensure-namespace-package
            ...
            *count-slot-suffix* *count-type-suffix* *xml-id-attribute*
            find-prefix-namespace parse-qualified-name find-xml-name intern-xml-name
            parse-as-type
            into-xml-element store-xml-content outof-xml-element pop-elements
            define-ignored-elements *ignored-elements* *assumptions*
...
(defnamespace :xsd "http://www.w3.org/2001/XMLSchema"
  ...
  complex-type #:sequence min-occurs max-occurs unbounded
  ...
  non-negative-integer date-time #:time date
...
(defnamespace :rs "http://reasoner.sourceforge.net/reasoner"
  ...
  variable attribute-reference name #:type
...
(defmethod find-xml-name ((name string) (namespace symbol) &optional (errorp t))
  (cond ((find-symbol name (ensure-namespace-package namespace)))
...
(defun find-datatype (tag content)
  (range-external-type (find-element-type tag content)))
...
(defmethod assimilate-content (element (element-content extended-object)
  ...
  (assimilate-content element
                      ...
                      (parse-content content
                                     (find-datatype tag element-content))))
...
(defmethod finalize-content (element (tag (eql 'xmlrs::comment)) (content string))
  ...
  ;; Preserve as string.
  (setf (element-content element) (string-trim '(#\Space #\Tab #\Newline) content)))
...
(defmethod assimilate-content (element element-content
                               subelement (tag (eql 'complex-type)) content)
  (declare (ignore element element-content))
...
(defun ensure-count-type (class-name slot-name numeric-range)
  (let ((count-type-name (if (symbolp numeric-range)
                             numeric-range
                           (case (range-min numeric-range)
    ...
    (unless count-type-name
      (setq count-type-name (add-lisp-name-suffix class-name
        ...
        ;; Define type.
        (ensure-range-class count-type-name :body numeric-range))
    count-type-name))
      ...
      (when count-slot-name
        (setq count-slot-specification (find count-slot-name specifications
            ...
            count-type-name (ensure-count-type class-name
                                               (getf specification :name)
                                               (getf count-slot-specification :type)))
...
(defmethod assimilate-content
  ...
  (let* ((class (retrieve-type element-content))
    ...
    (if name
        (ensure-class-using-class class name
                                  :direct-slots (finalize-slots name content)
                                  :metaclass *default-metaclass*)
      (call-next-method))))
...
(defun expand-slot-specification (specification)
  (let ((occurrence (getf specification :count))
        count-slot-name)
    (when occurrence
    ...
    (cond (type
           ...
           (let ((min-occurs (element-attribute element 'min-occurs :xsd))
             ...
             (if (or min-occurs max-occurs)
               ...
               (list (list :name name :type type)))))
          (t
           
           ;; Outer element. Pick up; flesh out.
           (setf (getf content :name) name)
           (expand-slot-specification content)))))
...
(defmethod assimilate-content (element element-content
  ...
  (let ((name (element-attribute element 'name :xsd))
    ...
    (ensure-range-class name
                        :options (if superclass `((:include ,superclass)))
...
(let ((element-stack ()))
  ...
  (defun into-xml-element (tag attribute-fn)
    (push (make-element :tag (get-lisp-name tag)
          ...
          element-stack)
    (unless (element-content (first element-stack))
