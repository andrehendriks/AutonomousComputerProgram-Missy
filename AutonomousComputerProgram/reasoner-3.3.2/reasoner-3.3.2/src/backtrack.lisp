    ...
    (values (if (or (if no-check
                    ...
                    (progn
                      (setq superset (cons a assumptions)
                            e (uniquify-environment tms
                                                    superset
                                                    dont-create
                                                    at-front))
                      (if no-check
                          nil
                        (subsumed-by-nogood-p (or e superset) tms 'ordered))))
                nil
              (or e superset))
            e)))

(defun subsumed-by-nogood-p (environment atms ordered)
  (or (contradictoryp environment)
      (let ((size (size environment)))
        (dolist (e (nogoods atms))
          (unless (< (size e) size)
            (return nil))
          (when (subsumesp e environment ordered)
